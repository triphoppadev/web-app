...
import { X, Package, ArrowRight, AlertTriangle, CheckCircle, Ship, Plane } from 'lucide-react'
      ...
      <div className="relative w-full sm:max-w-md bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
        ...
        {step === 'success' && (
          <div className="px-6 py-10 flex flex-col items-center text-center">
            ...
            <h3 className="text-xl font-bold text-[#392b75] mb-2">Booking Confirmed!</h3>
            <p className="text-sm text-gray-500 mb-1">{shipment.route}</p>
            <p className="text-sm text-gray-500 mb-6">
              Departing {format(new Date(shipment.departureDate), 'MMM d, yyyy')}
            </p>
            <div className="w-full p-4 rounded-2xl bg-[#f7f5ff] border border-[#e8d5e7] mb-6 text-left space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Freight mode</span>
                <span className="font-semibold text-[#392b75] capitalize">{freightMode}</span>
              </div>
              {freightMode === 'air' && (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Goods type</span>
                  <span className="font-semibold text-[#392b75] capitalize">{goodsType}</span>
                </div>
              )}
              <div className="flex justify-between text-sm">
                <span className="text-gray-500">Cargo</span>
                <span className="font-semibold text-[#392b75]">
                  {freightMode === 'sea' ? `${cbm} cbm` : `${kg} kg`}
                </span>
              </div>
              <div className="border-t border-[#e8d5e7] pt-2 flex justify-between">
                <span className="font-semibold text-[#392b75]">Total</span>
                <span className="font-bold text-[#96298d] text-lg">${totalPrice.toFixed(2)}</span>
              </div>
            </div>
            <button
              onClick={onClose}
        ...
        {step === 'confirm' && (
          <div className="px-6 py-5">
            <h4 className="font-bold text-[#392b75] mb-4">Confirm your booking</h4>
            <div className="space-y-3 mb-6">
              {[
                { label: 'Route', value: shipment.route },
                { label: 'Departure', value: format(new Date(shipment.departureDate), 'MMM d, yyyy') },
                { label: 'Freight mode', value: freightMode === 'air' ? 'Air Freight' : 'Sea Freight' },
                ...(freightMode === 'air' ? [
                  { label: 'Goods type', value: goodsType === 'special' ? 'Special goods' : 'Normal goods' },
                  { label: 'Weight', value: `${kg} kg` },
                  { label: 'Rate', value: `$${effectivePricePerKg}/kg` },
                  { label: 'Cargo cost', value: `$${(kg * effectivePricePerKg).toFixed(2)}` },
                ] : [
                  { label: 'Volume', value: `${cbm} cbm` },
                  { label: 'Rate', value: `$${shipment.pricePerCbm}/cbm` },
                  { label: 'Cargo cost', value: `$${(cbm * (shipment.pricePerCbm ?? 0)).toFixed(2)}` },
                  { label: 'Sea freight fee', value: `$${shipment.seaFreightProcessingFee?.toFixed(2)}` },
                ]),
                { label: 'Shipment processing fee', value: `$${processingFee.toFixed(2)}` },
              ].map(row => (
                <div key={row.label} className="flex justify-between text-sm">
                  <span className="text-gray-500">{row.label}</span>
                  <span className="font-medium text-[#392b75] capitalize">{row.value}</span>
                </div>
              ))}
              <div className="border-t border-[#e8d5e7] pt-3 flex justify-between">
                <span className="font-semibold text-[#392b75]">Total</span>
                <span className="font-bold text-[#96298d] text-lg">${totalPrice.toFixed(2)}</span>
              </div>
            </div>
            ...
            <p className="text-xs text-gray-400 mb-5">
              Payment collected separately. Booking marked <strong>pending</strong> until payment confirmed.
            </p>

            <div className="flex gap-3">
              <button
                onClick={() => setStep('form')}
                className="flex-1 py-3 rounded-xl border border-[#e8d5e7] text-sm font-medium text-[#392b75] hover:bg-[#f7f5ff] transition-colors"
              >
                Back
              </button>
              <button
        ...
        {step === 'form' && (
          <div className="px-6 py-5 space-y-5">
            ...
            {/* FREIGHT MODE — only for Dubai and China */}
            {hasSeaFreight && (
              <div>
            ...
            {/* GOODS TYPE — only for air freight on Dubai/China */}
              ...
              {freightMode === 'air' ? (
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">
                    {kg}kg × ${effectivePricePerKg}
                  </span>
                  <span className="font-medium text-[#392b75]">
                    ${(kg * effectivePricePerKg).toFixed(2)}
                  </span>
                </div>
              ) : (
                <>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">
                      {cbm} cbm × ${shipment.pricePerCbm}
                    </span>
                    <span className="font-medium text-[#392b75]">
                      ${(cbm * (shipment.pricePerCbm ?? 0)).toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-500">Sea freight processing</span>
                    <span className="font-medium text-[#392b75]">
                      ${shipment.seaFreightProcessingFee?.toFixed(2)}
                    </span>
                  </div>
                </>
              )}
